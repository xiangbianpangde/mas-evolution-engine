#!/usr/bin/env python3
"""
MAS Real v6.0 - Advanced MAS with Performance Optimization
...
class MASv6:
    """Advanced MAS with priority, retry, and monitoring"""
    ...
    def __init__(self, num_agents: int = 12):
        ...
        self.stats = {"submitted": 0, "completed": 0, "failed": 0, "verified": 0, "retried": 0}
        self.resource_log = []
    ...
    def submit(self, task: Task):
        ...
        # Priority is (priority_value, task) - lower number = higher priority
    ...
    def _handle_code(self, t: Task) -> str:
        templates = {
            ...
            "reverse": "s = 'Hello, World!'; print(s[::-1])\n",
        }
        code = "# Generated\n"
        ...
        with self.lock:
            ...
            return {
                **self.stats,
                ...
                "avg_task_time": total_time / max(1, self.stats["completed"]),
                "resource_log": self.resource_log[-10:],  # Last 10 samples
                "agents": {a.id: {"Name": a.name, "completed": a.completed, "failed": a.failed,
                                   "busy": f"{a.busy_time:.3f}s"}
...
def create_tasks() -> List[Task]:
    tasks = [
        # Normal priority
        Task(id="01", description="Count /etc/passwd", category="analysis", difficulty=1, priority=TaskPriority.NORMAL),
        Task(id="02", description="Check disk", category="analysis", difficulty=1, priority=TaskPriority.NORMAL),
        Task(id="03", description="Check memory", category="analysis", difficulty=1, priority=TaskPriority.NORMAL),
        Task(id="04", description="Generate hello", category="code", difficulty=1, priority=TaskPriority.NORMAL),
        Task(id="05", description="Generate sort", category="code", difficulty=1, priority=TaskPriority.NORMAL),
        
        # High priority
        Task(id="06", description="Generate fibonacci", category="code", difficulty=2, priority=TaskPriority.HIGH),
        Task(id="07", description="Generate primes", category="code", difficulty=2, priority=TaskPriority.HIGH),
        Task(id="08", description="Generate factorial", category="code", difficulty=2, priority=TaskPriority.HIGH),
        Task(id="09", description="Research /tmp", category="research", difficulty=1, priority=TaskPriority.NORMAL),
        Task(id="10", description="Research /var/log", category="research", difficulty=2, priority=TaskPriority.NORMAL),
        ...
        Task(id="13", description="Create plan", category="planning", difficulty=1, priority=TaskPriority.NORMAL),
        Task(id="14", description="Create deployment", category="planning", difficulty=2, priority=TaskPriority.HIGH),
        Task(id="15", description="Generate report", category="communication", difficulty=1, priority=TaskPriority.NORMAL),
        
        # More tasks
        Task(id="16", description="Optimize", category="optimization", difficulty=2, priority=TaskPriority.NORMAL),
        Task(id="17", description="Monitor", category="monitoring", difficulty=1, priority=TaskPriority.NORMAL),
        Task(id="18", description="Schedule", category="scheduling", difficulty=1, priority=TaskPriority.LOW),
        Task(id="19", description="Check CPU", category="analysis", difficulty=1, priority=TaskPriority.NORMAL),
        Task(id="20", description="Check load", category="analysis", difficulty=1, priority=TaskPriority.HIGH),
        
        # With dependencies
        Task(id="21", description="Post-research plan", category="planning", difficulty=2, priority=TaskPriority.NORMAL,
             depends_on=["10"]),
        Task(id="22", description="Post-analysis report", category="communication", difficulty=2, priority=TaskPriority.NORMAL,
             depends_on=["02", "03"]),
    ]
    return tasks
...
def main():
    print("=" * 70)
    print("MAS v6.0 - ADVANCED WITH PRIORITY, RETRY & MONITORING")
    print("=" * 70)
    
    mas = MASv6(num_agents=12)
    tasks = create_tasks()
    ...
    print(f"  {stats['retried']} retried")
    print(f"  Wall time: {total_time:.3f}s")
    print(f"  CPU time: {stats['total_time']:.3f}s")
    print(f"  Throughput: {stats['throughput']:.1f} tasks/sec")
    print(f"  Avg task time: {stats['avg_task_time']:.3f}s")
    
    print(f"\nAgents:")
    for aid, info in stats['agents'].items():
        print(f"  {info['Name']}: {info['completed']} ok, {info['failed']} fail, {info['busy']} busy")
    
    print(f"\nResource Monitor (last 5 samples):")
    for sample in stats['resource_log'][-5:]:
        print(f"  t={sample['time']:.0f}: cpu={sample['cpu_load']:.2f}, active={sample['active_agents']}, queue={sample['queue_size']}")
    
    print(f"\nTask Results:")
    for tid, t in mas.tasks.items():
        icon = {TaskStatus.COMPLETED: "✓", TaskStatus.VERIFIED: "✓✓", 
                TaskStatus.FAILED: "✗", TaskStatus.RETRIED: "⟳"}.get(t.status, "?")
        prio = t.priority.name[:1]
        dt = t.execution_time if t.end_time > 0 else 0
        result = (t.result or t.error or "...")[:45]
        deps = f" ←{t.depends_on}" if t.depends_on else ""
        retry = f" (retry {t.retries})" if t.retries > 0 else ""
        print(f"  {icon}[{prio}][{tid}]{deps}{retry} {t.category:15} ({dt:.3f}s) {result}...")
