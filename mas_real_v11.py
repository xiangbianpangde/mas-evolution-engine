...
class MASv11:
    ...
    def _route(self, task: Task) -> Optional[Agent]:
        with self.lock:
            cat_map = {"analysis": "analyzer", "code": "coder", "research": "researcher", "planning": "planner", "verification": "verifier", "communication": "communicator"}
...
def create_tasks() -> List[Task]:
    categories = ["analysis", "code", "research", "planning", "verification", "communication"]
    return [Task(id=f"{i:02d}", description=f"Task {i}", category=categories[i % len(categories)], difficulty=1) for i in range(1, 51)]

def main():
    print("=" * 50 + "\nMAS v11.0 - 50 TASKS\n" + "=" * 50)
    mas = MASv11(num_agents=12)
    tasks = create_tasks()
    print(f"Agents: {len(mas.agents)}, Tasks: {len(tasks)}")
    for t in tasks: mas.submit(t)
    start = time.time()
    t = threading.Thread(target=mas.run); t.start()
    t.join(timeout=120); mas.stop()
    stats = mas.get_stats()
