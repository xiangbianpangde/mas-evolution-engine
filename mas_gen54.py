...
class KnowledgeBase:
    ...
    def get_relevant_insights(self, query: str, limit: int = 5) -> List[str]:
        ...
        if not insights:
            return []
        return [i["text"] for i in insights[-limit:] if "quality" in i and i["quality"] >= 0.8]
...
if __name__ == "__main__":
    ...
    with open("/root/.openclaw/workspace/mas_gen54_test.log", "w") as f:
        f.write(f"Gen 54: q={result['avg_quality']:.3f}, success={result['success_rate']:.1f}%, eff={result['avg_tokens']:.0f}tok\n")
