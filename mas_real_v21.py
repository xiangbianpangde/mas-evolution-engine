#!/usr/bin/env python3
"""MAS v21 - Enhanced with retry logic"""
...
class MASv21:
    ...
    def _handle_analysis(self, t: Task) -> str:
        ...
        for key, cmd in cmds.items():
            if key in t.description.lower():
                try:
                    r = subprocess.run(cmd, capture_output=True, text=True, timeout=max(5, t.timeout))
    ...
    def _handle_code(self, t: Task) -> str:
        ...
        try:
            with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
                f.write(code)
                path = f.name
            r = subprocess.run(["python3", path], capture_output=True, text=True, timeout=max(5, t.timeout))
            os.unlink(path)
    ...
    def _handle_security(self, t: Task) -> str:
        try:
            r = subprocess.run(["ls", "-la", "/tmp"], capture_output=True, text=True, timeout=max(5, t.timeout))
            return f"Security: {len(r.stdout.splitlines())} entries"
        except: return "Security: timeout"
    ...
    def get_stats(self) -> Dict[str, Any]:
        with self.lock:
            ...
            return {**self.stats, "total_time": total_time, "throughput": self.stats["completed"] / max(0.001, total_time),
                    "agents": {a.id: {"Name": a.name, "completed": a.completed, "failed": a.failed} for a in self.agents.values()}}
...
def create_tasks() -> List[Task]:
    ...
    for i in range(1, 501):
        ...
        tasks.append(Task(id=f"{i:03d}", description=f"Task {i}", category=cat, difficulty=1, priority=priority, depends_on=depends))
    return tasks

def main():
    print("=" * 50 + "\nMAS v21.0 - 500 TASKS\n" + "=" * 50)
    mas = MASv21(num_agents=16)
    tasks = create_tasks()
    ...
    for aname, adata in stats['agents'].items():
        if adata['completed'] > 0:
            print(f"  {adata['Name']}: {adata['completed']} ok, {adata.get('failed',0)} failed")
